...
안녕하세요. 셀러리 컴퍼니에서 [직고래](https://play.google.com/store/apps/details?id=com.sellerleecompany.jikgorae)를 개발하고 있는 스티치입니다.

이번에 저희 프로젝트에서 코드 커버리지 분석 도구를 적용하게 되었습니다. 이번 글에서는 **코드 커버리지가 무엇인지, 그리고 저희 프로젝트에서는 어떤 코드 커버리지 분석 도구를 적용할 것인지**에 관해 이야기해보도록 하겠습니다.

시작하기에 앞서 **테스트 코드**에 대해 간단하게 설명하고 들어가도록 하겠습니다. 테스트 코드는 이번에 알아볼 코드 커버리지와 밀접한 관련이 있기 때문입니다.

먼저 저희는 프로젝트를 진행하면서 꾸준히 테스트 코드를 작성하고 있습니다. 그런데 **테스트 코드를 작성하는 이유**가 무엇일까요?

테스트 코드를 작성함으로써 얻을 수 있는 장점은 정말 많습니다. 몇 가지만 이야기해보면
...
그런데 과연 우리가 테스트 코드를 잘 작성하고 있는지 **어떻게 확인할 수 있을까요?** '기능 추가 및 수정으로 인한 부작용을 줄여준다'라고 했는데, 혹시나 테스트 코드 작성을 빠트린 부분이 발생한다면 어떻게 될까요?

테스트 코드가 작성되지 않은 **로직에 대한 변경**이 발생하고 이로 인해 **부작용이 발생**했다고 가정해봅시다. 실수로 테스트 코드를 작성하지 못했고, 그로 인해 부작용이 발생한다는 사실을 파악하지 못한다면 어떡할까요?
